...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String createTableStatement = "CREATE TABLE " + TASKS_TABLE + " (" + COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT," + COLUMN_OPIS + " TEXT, " + COLUMN_KIEDYKONIEC + " TEXT, " + COLUMN_STATUS + " TEXT)";
    ...
    public List<TaskModel> getEveryOne()
    {
        ...
        String querystring = "SELECT * FROM "+TASKS_TABLE+" ORDER BY " + COLUMN_KIEDYKONIEC;
    ...
    public List<String> getEveryOneSimple()
    {
        ...
        if(cursor.moveToFirst())
        {
            do
            {
                ...
                TaskModel taskmodel = new TaskModel(taskid,taskopis,taskkiedykoniec,taskstatus);
                resultList.add(taskmodel.getId()+" "+taskmodel.getOpis()+ " " + taskmodel.getKiedykoniec());
            } while (cursor.moveToNext() );
        }
        else
        {
    ...
    public boolean deleteOneSimple( String taskModel)
    {
        Integer id=Integer.parseInt(taskModel.split(" ")[0]);
        SQLiteDatabase db = this.getReadableDatabase();
        String querystring = "DELETE FROM "+TASKS_TABLE +" WHERE "+COLUMN_ID + "=" +id.toString();
        Cursor cursor = db.rawQuery(querystring,null);
        if (cursor.moveToFirst())
        {
            return true;
